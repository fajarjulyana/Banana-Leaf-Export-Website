...
# Exchange rate (USD to IDR) - in production, this should be fetched from an API
...
def get_product_price(product, currency='USD'):
    """Get product price in specified currency with dynamic conversion"""
    if currency == 'IDR':
        # If product has IDR price, use it; otherwise convert from USD
        if hasattr(product, 'price_idr') and product.price_idr and product.price_idr > 0:
            return product.price_idr
        else:
            return product.price_usd * USD_TO_IDR_RATE
    else:
        # If product has USD price, use it; otherwise convert from IDR
        if hasattr(product, 'price_usd') and product.price_usd and product.price_usd > 0:
            return product.price_usd
        else:
            return product.price_idr / USD_TO_IDR_RATE if hasattr(product, 'price_idr') and product.price_idr else 0
