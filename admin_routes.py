...
def settings():
    ...
    if request.method == 'POST':
        ...
        # Handle gallery images upload (max 5 images)
        gallery_files = request.files.getlist('gallery_files')
        gallery_urls = []

        # Get existing gallery images count
        existing_gallery_count = 0
        if settings.gallery_images:
            existing_urls = [url.strip() for url in settings.gallery_images.split(',') if url.strip()]
            existing_gallery_count = len(existing_urls)

        # Calculate how many new images we can add
        max_new_images = 5 - existing_gallery_count

        if max_new_images > 0:
            for i, file in enumerate(gallery_files[:max_new_images]):
                if file and file.filename:
                    filename = secure_filename(file.filename)
                    unique_filename = f"{uuid.uuid4().hex}_{filename}"
                    file_path = os.path.join(UPLOAD_FOLDER, unique_filename)
                    file.save(file_path)
                    gallery_urls.append(f"/static/uploads/{unique_filename}")

            if gallery_urls:
                if settings.gallery_images:
                    existing_urls = [url.strip() for url in settings.gallery_images.split(',') if url.strip()]
                    existing_urls.extend(gallery_urls)
                    # Ensure we don't exceed 5 images total
                    settings.gallery_images = ','.join(existing_urls[:5])
                else:
                    settings.gallery_images = ','.join(gallery_urls)

                flash(f'Successfully uploaded {len(gallery_urls)} gallery images', 'success')

            if len(gallery_files) > max_new_images:
                flash(f'Maximum 5 gallery images allowed. Only {max_new_images} images were uploaded.', 'warning')
        else:
            flash('Maximum gallery limit (5 images) reached. Please delete some images first.', 'warning')
        ...
        settings.theme_mode = request.form.get('theme_mode', 'preset')
        if settings.theme_mode == 'preset':
            settings.selected_theme = request.form.get('selected_theme', 'nature_life')
        else:
            settings.selected_theme = 'custom'
            settings.custom_primary_color = request.form.get('custom_primary_color', '#28a745')
            settings.custom_secondary_color = request.form.get('custom_secondary_color', '#6c757d')
            settings.custom_accent_color = request.form.get('custom_accent_color', '#17a2b8')

        # Legacy appearance settings (for backward compatibility)
        settings.primary_color = request.form.get('custom_primary_color', '#28a745')
        settings.secondary_color = request.form.get('custom_secondary_color', '#6c757d')
        ...
        settings.gallery_mode = request.form.get('gallery_mode', 'static')
